...
echo "==> Checking containerlab lab status"
clab inspect -t apnic62-wan.clab.yml

echo ""
echo "==> Checking IS-IS adjacencies on r1-p1"
docker exec clab-apnic62-wan-r1-p1 sr_cli -c \
  "show network-instance default protocols isis adjacency" || true

echo ""
echo "==> Checking IS-IS adjacencies on r5-pe1"
docker exec clab-apnic62-wan-r5-pe1 sr_cli -c \
  "show network-instance default protocols isis adjacency" || true
