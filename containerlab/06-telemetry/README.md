# Containerlab Workshop: Activity 6 - Nokia SR Linux Telemetry Lab

This activity simply gives you access to an existing lab demonstrating telemetry capabilities in a data center topology. This lab is officially available here - [srl-telemetry-lab](https://github.com/srl-labs/srl-telemetry-lab).

Observe how the different components are deployed with Containerlab.

Access the Grafana dashboard and follow the instructions to start traffic in the fabric.

Can you figure out how to shut down a link ? You should then see on Grafana how traffic is redirected across the fabric.
...
* Grafana: <http://localhost:3000>. Anonymous access is enabled; no credentials are required. In the workshop portal, open Grafana from the **Grafana** link on your lab card instead (the lab shell also has it in `$GRAFANA_ROOT_URL`). If you want to act as an admin, use `admin/admin` credentials.
